function network = backpropMe(network, X, T, max_iter, noise, verbos,lambda)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    for iter=1:max_iter
        if verbos, disp([' - iteration ' num2str(iter) ' of ' num2str(max_iter) '...']); end;
...
%             checkgrad('backprop_gradient', v, 1e-5, network, tmpX, tmpT)
            [v, fX] = minimizeMe(v, 'backprop_gradient', 3, network, tmpX, tmpT, lambda);
            ...
            if isempty(fX)
                ...
                C = sum((T(:) - reconX(:)) .^ 2) ./ n;
                if verbos, disp([' - final noisy MSE: ' num2str(C)]); end;
                return
            end
        end
        ...
        reconX = run_data_through_autoenc(network, X);
        C = sum((T(:) - reconX(:)) .^ 2) ./ n;
        if verbos, disp([' - current noisy MSE: ' num2str(C)]); end;
    end
