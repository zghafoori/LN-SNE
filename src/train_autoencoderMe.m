function [network, mappedX, reconX] = train_autoencoderMe(X, layers, noise, max_iter, verbos)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    network = backpropMe(network, X, X, max_iter, noise,verbos);
    
    % Get representation from hidden layer
    [reconX, mappedX] = run_data_through_autoenc(network, X);
    if verbos, disp(['Final MSE of reconstructions: ' num2str(mean((X(:) - reconX(:)) .^ 2))]); end;
