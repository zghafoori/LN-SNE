function [model, mappedX] = train_deep_autoencMe(X, layers, lambda,max_iter,noise,verbos)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    for i=1:no_layers
        %noise = 0.1;
        %max_iter = 30;
        [network, mappedX] = train_autoencoderMe(mappedX, layers(i), noise, max_iter, verbos);
    ...
    %noise = 0.1;
    %max_iter = 30;
    model = backpropMe(model, X, X, max_iter, noise, lambda,verbos);
    
    % Compute mean squared error of final model predictions
    [reconX, mappedX] = run_data_through_autoenc(model, X);
    if verbos, disp(['MSE of final model: ' num2str(size(X, 2) .* mean((reconX(:) - X(:)) .^ 2))]); end;
end
