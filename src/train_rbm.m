...
%     machine.W = (rand(v, h) - 0.5) * 4 * sqrt(6./ (v + h));
%     machine.bias_upW = (rand(1, h) - 0.5) * 4 * sqrt(6./ (h+1));
%     machine.bias_downW = (rand(1, v) - 0.5) * 4 * sqrt(6./ (v+1));
    
%     machine.W = randn(v, h) * 0.01;
...
%     machine.bias_downW = zeros(1, v);
    ...
    for iter=1:max_iter
        ...
        if verbos, disp(['Iteration ' num2str(iter) ' (rec. error = ' num2str(err) ')...']); end;
    end
    disp(' ');
